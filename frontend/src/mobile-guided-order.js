const guidedOrder = { booted: false, step: 1, touched: false };
...
function setStep(step) {
  ...
  requestAnimationFrame(applyGuidedFlow);
}
...
function installClickFlow() {
  ...
  document.addEventListener('click', (event) => {
    ...
    const kind = cardKind(card);
    if (kind === 'snack') setTimeout(() => { setStep(2); focusCorrectArea(2); }, 220);
    if (kind === 'drink') setTimeout(() => { setStep(4); focusCorrectArea(4); }, 220);
  }, true);
}
...
function bootGuidedOrder() {
  ...
  installClickFlow();
  setInterval(applyGuidedFlow, 450);
}
